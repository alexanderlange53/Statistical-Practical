...
In der Tabelle sind nicht nur die Kategorienanzahlen der Variablen, sondern auch die vermuteten Formen der Einflüsse 
der Kovariablen auf die abhängigen Variablen aufgelistet. Diese wurden durch visuelle Darstellung aller Kovariablen über 
den abhängigen Variablen ermittelt. Nach visueller Einschätzung ergab sich, dass alle nominal skalierte Variablen, wie 
z.B. die Nationalität, parametrisch linear und dass alle kardinal skalierte Variablen, wie z.B. die Altersklasse 
des Befragten, offensichtlich als nicht parametrisch modelliert werden sollten. Diese beobachteten Zusammenhänge finden 
sich häufig in Regressionsmodellen, da eine lineare Modelle oft nicht flexibel genug sind um kardinal oder 
metrisch skalierte Zusammenhänge beschreiben zu können \cite[p. 9]{fahrmeir2009regression}. In Anlehnung an 
\cite[p. 503 ff., p. 524 ff.]{fahrmeir2013regression} wird der räumliche Effekt entweder kontinuierlich als Tensor 
Produkt oder diskret als Gauß-Markov Zufallsfeld modelliert.\\
...
Punktschätzung zur Unsicherheit und somit ein weiteres Modellgütemerkmal \cite{fahrmeir2013regression}[p. 471]. Des 
...
Zweck wurde eine \textit{Leave-One-Out} Kreuzvalidierung durchgeführt \cite{fahrmeir2013regression}[p. 149], in welcher 
...
\section{Ergebnisse}
textcolor{red}{Da Modelle ohne räumlichen Effekt in der AIC-Untersuchung und Kreuzvalidierung stets schlechter 
