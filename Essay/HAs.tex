...
Da alle zu modellierenden endogenen Variablen eine kategorielle Response zeigen, folglich eine Ausprägungswahrscheinlichkeit der endogenen Variablen modelliert werden soll, kommen direkte, sind untransoformierte Regressionsmodelle nicht geeignet. Für die Fragestellung dieser Arbeit kommen nur Regressionsmodelle in Frage, die eine Modellierung Ausprägungswahrscheinlichkeit der Response-Variable im Wertebereich zwischen 0 und 1 ermöglichen. In generalisierten linearen Modelle wird der lineare Prädiktor $\eta$ über durch die Response-Funktion $h$ transformiert, um diese Restriktion zu gewährleisten. Als Transformationsfunktion kommt jede monoton ansteigende Funktion in Frage. Die Auswahl hängt in erster Linie von der skalierung der Response-Variable ab. Bei binärer Response-Variable wird häufig die Logit Response-Funktion gewählt \cite[p. 270 f.]{fahrmeir2009regression}. \textcolor{red}{weißt du, wie man die Formel nummeriert?}
$$h(\eta)=\frac{exp(\eta)}{1+exp(\eta)}$$
Bei ordinalskalierter Response kann die Reihenfolge der Ausprägung im generalisierten Modell explizit berücksichtigt werden. Im kumulativen Modell wird angenommen, dass für jeden Datensatz eine kontinuierliche (unbekannte) latente Variable $u_i$ existiert, deren Ausprägung die Kategoriezugehörigkeit bestimmt. Unter der Annahme, dass diese latente Variable über einen linearen Prädiktor ohne Intercept vorhergesagt werden kann, existieren Schwellenwerte bei welchen

Die exogenen Variablen, mit denen das Modell für die räumliche Extrapolation parametrisiert werden soll, sind sehr heterogen. Es kommen sowohl nominal als auch kardinalskalierte sozioökonomischen Variablen in Frage. Während ordinalskalierte Variablen häufig als linearer Prädiktor in ein generalisiertes Modell eingehen, zeigen kardinal skalierte Variablen oft einen nichtlinearen Einfluss \cite[p. 9]{fahrmeir2009regression}. Die in generalisierten linearen Modellen vorausgesetzte lineare Abhängigkeit zwischen allen transformierten Kovariablen und der Response-Variable in der Form
$$\eta_{i} =\mathbf{x'_i} \boldsymbol{\beta}$$
ist demnach für die Daten dieser Arbeit zu unflexibel. Bei bestehender Datenlage haben flexiblere Modelle also voraussichtlich Vorteile gegenüber reinen linearen Modellen.

\textcolor{red}{intercept ?ocat}



Generalisierte additive Modelle (GAM) sind für die kleinräumige Extrapolation vorteilhaft, da sie unterschiedliche strukturierte Effekte sehr flexibel verbinden können. Da sowohl nominal als auch kardinal skalierte Kovariableneffekte in das Modell eingehen sollen (Abbildung \ref{endogene}, Tabelle \ref{Datensatz}) und da eine lineare Abhängigkeit, welche bei generalisierten linearen Modellen vorausgesetzt wird,
ocat, gam \cite{wood2016}
kardinal skalierte Daten häufig
, dass lineare Abhängigkeiten, die bei generalisierten linearen Modellen vorausgesetzt werden, nicht ausreichen um alle Effekte zu beschreiben.
