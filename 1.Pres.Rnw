...
require(rgdal);require(rgeos);require(ggmap) # package loading
...
# Haushaltsgröße
count1 <- as.data.frame(table(Grund$Haushaltsgroesse))
count2 <- as.data.frame(table(Stich$Haushaltsgroesse))
Haushaltsgroesse <- cbind(count1[1], count1[,2]/sum(count1[,2]), (count2[,2])/sum(count2[,2]))
names(Haushaltsgroesse) <- c('Haushaltsgroesse', 'Grundgesamtheit', 'Stichprobe')
Haushaltsgroesse <- melt(Haushaltsgroesse, id = 'Haushaltsgroesse')
b3 <- ggplot(Haushaltsgroesse, aes(x = Haushaltsgroesse, y = value, fill = variable)) +
  geom_bar(stat="identity", position=position_dodge(), col = 'black', alpha = .5) +
  scale_fill_manual(values=c(colo[1], colo[3])) + ylab('Relative Häufigkeit') + xlab('Haushaltsgröße') +
  theme(legend.position = 'none', legend.title = element_blank())
...
\begin{frame}
\begin{itemize}
\item Penalised-Spline (Elmar slides)
\item Tensor Penalised spline (Elmar slides)
\end{itemize}
